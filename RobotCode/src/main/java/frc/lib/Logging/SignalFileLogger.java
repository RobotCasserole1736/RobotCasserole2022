 ...
 *       new file.
 *   <li>Once per loop, call the writeData() method with the full list of values to write to file
 *       (all must be converted to doubles).
 *   <li>During DisabledInit, call the close() method to close out any file which was being written
 *       to while the robot was doing something.
 *   <li>Post-match or -practice, extract the data logs from the USB drive(maybe using FTP?) and
 *       view with excel or your favourite software.
 * </ol>
 */
public class SignalFileLogger {
  ...
  private void writeLogData(DataSample samp_in) {
    ...
    if (idx != null) {
      ...
      System.out.println(
          "Developer Error - Attempt to log value from signal \""
  ...
  /**
   * Write a list of doubles to the output file, assuming it's open. Creates a new line in the .csv
   * log file.
   *
