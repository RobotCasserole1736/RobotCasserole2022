...
public class AutoChooserConfig extends VisibleWidgetConfig {
  ...
  public String getJSDeclaration() {
    String retStr =
        String.format(
            "var widget%d = new AutoChooser('widget%d', '%s', %s, onWidget%dValUpdated);\n",
            idx, idx, name, getJsModeNameListString(), idx);
  ...
  public String getJSCallback() {
    ...
    retStr +=
        String.format(
            "    nt4Client.addSample(\"%s\", nt4Client.getServerTime_us(), value);\n",
            nt4TopicDesVal);
    retStr += "}";
    return retStr;
  }
