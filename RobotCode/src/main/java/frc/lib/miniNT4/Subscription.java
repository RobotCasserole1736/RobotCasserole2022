...
public class Subscription {
  ...
  HashMap<Topic, LinkedList<TimestampedValue>> sampleQueues =
      new HashMap<Topic, LinkedList<TimestampedValue>>();
  Thread txThread;
  ...
  public void start() {
                ...
                try {
                  while (backgroundThreadRunCmd) {
                    ...
                    long sleepDur =
                        Math.max(
                            5l,
                            (long) (periodicTxRate_sec * 1000.0)
    ...
    txThread.setName(
        "NT4 Subscription " + clientRef.friendlyName + " UID " + Integer.toString(subuid));
