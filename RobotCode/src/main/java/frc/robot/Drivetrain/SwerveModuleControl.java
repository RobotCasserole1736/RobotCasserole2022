...
class SwerveModuleControl {
  ...
  // TODO create an "Azimuth Controller" which calculates steer motor voltage commands from
  // actual/desired angles

  public SwerveModuleControl(
      String posId, int wheelMotorIdx, int azmthMotorIdx, int azmthEncoderIdx) {
  ...
  public void update(double curSpeedFtPerSec, double maxAzmthErr_deg) {
    ...
    // TODO update the "actualState" object with this module's present steer angle and speed in
    // meters per second
  ...
  public void updateTelemetry() {
    double sampleTime =
        Timer.getFPGATimestamp(); // TODO - this should actually be coming from the loop timing
    // utility, whenever Lucas finishes it up.
