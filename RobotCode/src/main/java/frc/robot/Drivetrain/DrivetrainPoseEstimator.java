...
public class DrivetrainPoseEstimator {
  ...
  Pose2d curEstPose =
      new Pose2d(
          Constants.DFLT_START_POSE.getTranslation(), Constants.DFLT_START_POSE.getRotation());
  ...
  public void update() {
    ...
    // Based on gyro and measured module speeds and positions, estimate where our robot should have
    // moved to.
    SwerveModuleState[] states = DrivetrainControl.getInstance().getModuleActualStates();
    Pose2d prevEstPose = curEstPose;
    curEstPose =
        m_poseEstimator.update(getGyroHeading(), states[0], states[1], states[2], states[3]);
