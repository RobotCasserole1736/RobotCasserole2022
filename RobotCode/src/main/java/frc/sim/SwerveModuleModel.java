...
class SwerveModuleModel {
  ...
  final double MODULE_NORMAL_FORCE_N =
      Constants.ROBOT_MASS_kg
          * 9.81
          / Constants.NUM_MODULES; // Assume weight evenly distributed between all modules.
  ...
  public SwerveModuleModel(int wheelMotorIdx, int azmthMotorIdx, int azmthEncIdx) {
    ...
    wheelMotor =
        new MotorGearboxWheelSim(
            DCMotor.getNEO(1),
            WHEEL_GEAR_RATIO,
            Units.inchesToMeters(Constants.WHEEL_RADIUS_IN * 2),
            0.01);
    azmthMotor =
        new SimpleMotorWithMassModel(
            DCMotor.getVex775Pro(1), AZMTH_GEAR_RATIO, AZMTH_EFFECTIVE_MOI);
  ...
  public void update(boolean isDisabled, double batteryVoltage) {
    ...
    wheelMotorCtrl.sim_setActualPosition(
        Units.rotationsToRadians(wheelMotor.getMotorPosition_Rev()));
    azmthMotorCtrl.sim_setActualPosition(
        Units.rotationsToRadians(azmthMotor.getMotorPosition_Rev()));
  ...
  public ForceAtPose2d getCrossTreadFrictionalForce(Force2d netForce_in) {
    ...
    if (Math.abs(crossTreadForceMag) > WHEEL_MAX_STATIC_FRC_FORCE_N
        || Math.abs(crossTreadVelMag) > 0.001) {
