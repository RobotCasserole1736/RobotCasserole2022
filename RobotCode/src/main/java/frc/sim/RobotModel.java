...
public class RobotModel {
  ...
  public void update(boolean isDisabled) {
    ...
    for (long count = 0; count < numIter; count++) {
      ...
      currentDraw_A =
          QUIESCENT_CURRENT_DRAW_A
              + dt.getCurrentDraw()
              + shooter.getCurrentDraw_A()
              + intake.getCurrentDraw_A()
              + ps.getCurrentDraw_A();

      // batteryVoltage_V = BatterySim.calculateLoadedBatteryVoltage(BATTERY_NOMINAL_VOLTAGE,
      // BATTERY_NOMINAL_RESISTANCE, currentDraw_A);
