...
class DrivetrainModel {
  ...
  Pose2d
      endRobotRefFrame; // Previous sent pose, allows us to detect when the robot has been manually
  // moved.
  ...
  public void update(boolean isDisabled, double batteryVoltage) {
    ...
    Pose2d startRobotRefFrame =
        field.getRobotPose(); // origin on and aligned to robot's present position in the field
    ...
    for (int idx = 0; idx < Constants.NUM_MODULES; idx++) {
      Pose2d modPose =
          fieldReferenceFrame
              .transformBy(fieldToRobotTrans)
              .transformBy(Constants.robotToModuleTF.get(idx));
    ...
    ArrayList<ForceAtPose2d> wheelMotiveForces =
        new ArrayList<ForceAtPose2d>(Constants.NUM_MODULES);
    for (int idx = 0; idx < Constants.NUM_MODULES; idx++) {
        ...
        (ForceAtPose2d mf) -> {
          preFricNetForce.accum(
              mf.getForceInRefFrame(
                  startRobotRefFrame)); // Add up all the forces that friction gets a chance to
          // fight against
        });
    ...
    ForceAtPose2d preFricNetForceRobotCenter =
        new ForceAtPose2d(preFricNetForce, startRobotRefFrame);

    // Calculate the forces from cross-tread friction at each module
    ArrayList<ForceAtPose2d> netXtreadFricForces =
        new ArrayList<ForceAtPose2d>(Constants.NUM_MODULES);
    for (int idx = 0; idx < Constants.NUM_MODULES; idx++) {
      SwerveModuleModel mod = modules.get(idx);
      double perWheelForceFrac =
          1.0 / Constants.NUM_MODULES; // Assume force evenly applied to all modules.
    ...
    Vector2d velocity =
        new Vector2d(
            vel_prev.x
                + (accel.x + accel_prev.x)
                    / 2
                    * Constants.SIM_SAMPLE_RATE_SEC, // Trapezoidal integration
            vel_prev.y + (accel.y + accel_prev.y) / 2 * Constants.SIM_SAMPLE_RATE_SEC);

    Translation2d posChange =
        new Translation2d(
            (velocity.x + vel_prev.x)
                / 2
                * Constants.SIM_SAMPLE_RATE_SEC, // Trapezoidal integration
